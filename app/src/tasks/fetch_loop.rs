...
use crate::services::{
    ...
    loader::{LoaderBsc, LoaderBtc, LoaderEth, LoaderTron},
...
pub async fn run_eth_loop(config: AppConfig) -> Result<()> {
    ...
    // شروع fetch
    ethereum::fetcher::fetch_eth(loader.clone(), start_block, config.total_eth_txs).await?;
...
pub async fn run_bsc_loop(config: AppConfig) -> Result<()> {
    ...
    // شروع fetch
    bsc::fetch_bsc(loader.clone(), start_block, config.total_bsc_txs).await?;
...
pub async fn run_tron_loop(config: AppConfig) -> Result<()> {
    ...
    let last_synced = get_last_synced_block(&loader.clickhouse, "tron").await?;
